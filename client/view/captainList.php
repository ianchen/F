...
$t = $_GET['t'];
$signupRecord = array();
$query = "SELECT * FROM `group` WHERE `CAPTAIN` = $uid AND `DATE` < NOW()";
...
for($i = 0; $i < count($signupRecord); $i++){
  $groupInfo = getGroupInfoByGid($signupRecord[$i]['GID']);
  $restaurantInfo = getRestaurantInfoByGid($signupRecord[$i]['GID']);
  echo "<tr>";
  ...
  echo "<a>取消報名</a>&nbsp;&nbsp;&nbsp;&nbsp;";
  echo "<a href=\"captainDetail.php?gid={$groupInfo['GID']}&uid=$uid&t=$t\">詳細內容</a>";
  echo "</td>";
  echo "</tr>";
  echo "<tr>";
  echo "<td height=\"30\" align=\"left\" valign=\"top\" class=\"dotline_01\">&nbsp;</td>";
